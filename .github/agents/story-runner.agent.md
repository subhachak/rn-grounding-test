...
5. **Devices.** Tell the person this takes several minutes per platform, then call `run_on_device` for `android`, then `ios`. It may ask the person to approve newly validated fallbacks. If a platform fails to run, report why and continue.
